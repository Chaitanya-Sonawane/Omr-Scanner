...
def classify_question(features, threshold, score_range, image_quality=None):
    ...
    # and aren't clearly separated from each other -> MULTI.
    #
    # Key insight for WhatsApp/low-res photos: on a compressed photo all
    # four blank bubbles can have slightly different darkness due to JPEG
    # blocking/compression noise. This pushes the second-highest combined_z
    # above blank_bar even when there is only ONE real mark, triggering a
    # false MULTI. To prevent this:
    #   - The second option must clear a HIGHER independent bar (multi_bar)
    #     than just blank_bar — it needs to look like a genuine mark, not
    #     just the noisiest blank.
    #   - AND the gap between best and second must be small (< gap_limit).
    #     A real single mark always produces a clearly larger best_z than
    #     second_z because it is the ONLY option that moved — the three
    #     blanks cluster together.  A genuine double-mark produces two
    #     options that are both high AND close together.
    # multi_bar is deliberately higher than blank_bar so compression noise
    # on blank bubbles (which peaks at ~0.5–0.8 z on these photos) doesn't
    # qualify a blank as a "second filled option".
    multi_bar  = 0.85 if not degraded else 0.75
    gap_limit  = 0.60 if not degraded else 0.50
    z_gap = best_z - second_z
    if second_z > multi_bar and z_gap < gap_limit:
        confidence = 100.0 * (1.0 - np.clip(z_gap / gap_limit, 0.0, 1.0))
